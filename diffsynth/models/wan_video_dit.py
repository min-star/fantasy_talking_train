...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs):
        b, s, n, d = *x.shape[:2], self.num_heads, self.head_dim  # grid_sizes 和 freqs用于旋转位置编码 b=1,s=21504,n=40,d=128

        # query, key, value function
        def qkv_fn(x):
            q = self.norm_q(self.q(x).to(torch.bfloat16)).view(b, s, n, d).to(torch.bfloat16)  
            k = self.norm_k(self.k(x).to(torch.bfloat16)).view(b, s, n, d).to(torch.bfloat16)
            v = self.v(x).view(b, s, n, d)
            return q, k, v

        q, k, v = qkv_fn(x)  # torch.Size([1, 21504, 40, 128])
        ...
        # output
        x = x.flatten(2) # torch.Size([1, 21504, 40, 128])
        x = self.o(x)
        return x
...
class WanAttentionBlock(nn.Module):
    def __init__(
        ...
        )  # <class 'diffsynth.models.wan_video_dit.WanI2VCrossAttention'>
    ...
    def forward(
        ...
        **kwargs,
        ...
        with amp.autocast(dtype=torch.bfloat16, device_type="cuda"):
            e = (self.modulation.to(dtype=e.dtype, device=e.device) + e).chunk(6, dim=1)  # 长度为6的元组，包括6个调制参数张量 [1,1,5120]
        # assert e[0].dtype == torch.float32

        # self-attention
        # y = self.self_attn(
        #     self.norm1(x).float() * (1 + e[1]) + e[0], seq_lens, grid_sizes, freqs
        # )  # 对x进行归一化，乘以调制参数e[1]，加上偏置e[0]，然后传入自注意力层  torch.Size([1, 21504, 5120])
            y = self.self_attn(
                self.norm1(x).bfloat16() * (1 + e[1]) + e[0], seq_lens, grid_sizes, freqs
        )  # 对x进行归一化，乘以调制参数e[1]，加上偏置e[0]，然后传入自注意力层  torch.Size([1, 21504, 5120])
        # with amp.autocast(dtype=torch.float32, device_type="cuda"):
        with amp.autocast(dtype=torch.bfloat16, device_type="cuda"):
            x = x + y * e[2]  # 将自注意力层的输出y乘以调制参数e[2]，然后加到x上

        # cross-attention & ffn function
        def cross_attn_ffn(x, context, context_lens, e, **kwargs):
            with amp.autocast(dtype=torch.bfloat16, device_type="cuda"):
                x = x + self.cross_attn(self.norm3(x), context, context_lens, **kwargs)  # cross_attn 表示交叉注意力层，这里会调自定义的attention_processor来处理
            # y = self.ffn(self.norm2(x).float() * (1 + e[4]) + e[3])  # 对x进行归一化，乘以调制参数e[4]，加上偏置e[3]，然后传入前馈网络层
                y = self.ffn(self.norm2(x).bfloat16() * (1 + e[4]) + e[3])
            # with amp.autocast(dtype=torch.float32, device_type="cuda"):
            with amp.autocast(dtype=torch.bfloat16, device_type="cuda"):
                x = x + y * e[5]  # 将前馈网络层的输出y乘以调制参数e[5]，然后加到x上
            return x

        x = cross_attn_ffn(x, context, context_lens, e, **kwargs).to(torch.bfloat16)  # 调用交叉注意力和前馈网络函数
        return x
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.freqs.device != device:
            self.freqs = self.freqs.to(device)  # torch.Size([1024, 64])

        if y is not None:
            x = [torch.cat([u, v], dim=0) for u, v in zip(x, y)]  # list：torch.Size([36, 21, 64, 64])

        # embeddings
        x = [self.patch_embedding(u.unsqueeze(0)) for u in x]  # torch.Size([1, 5120, 21, 32, 32]) 
        grid_sizes = torch.stack(
            [torch.tensor(u.shape[2:], dtype=torch.long) for u in x]
        )  # [B,2] tensor([[21, 32, 32]])
        x = [u.flatten(2).transpose(1, 2) for u in x]  # [[C, L, T],,]   # torch.Size([1, 21504, 5120])
        # print(f"x0.shape:{x[0].shape}")
        seq_lens = torch.tensor([u.size(1) for u in x], dtype=torch.long)  # tensor([21504])
        ...
        with amp.autocast(dtype=torch.float32, device_type="cuda"):
            ...
            e0 = self.time_projection(e).unflatten(1, (6, self.dim))  # torch.Size([1, 6, 5120])
        ...
        if clip_fea is not None:
            context_clip = self.img_emb(clip_fea)  # bs x 257 x dim  torch.Size([1, 257, 1280]) -> torch.Size([1, 257, 5120])
            context = torch.concat([context_clip, context], dim=1)  # torch.Size([1, 769, 5120])
