...
class WanPrompter(BasePrompter):
    ...
    def encode_prompt(self, prompt, positive=True, device="cuda"):
        ...
        ids, mask = self.tokenizer(prompt, return_mask=True, add_special_tokens=True)  # id:[1, 512], mask:[1, 512]
        ids = ids.to(device)
        mask = mask.to(device)
        seq_lens = mask.gt(0).sum(dim=1).long()  # tensor([15], device='cuda:0'),词数越多，seq_lens越大
        prompt_emb = self.text_encoder(ids, mask)  # torch.Size([1, 152, 4096]) 
